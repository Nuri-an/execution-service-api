...
export class RabbitMqEventBus
  implements OnModuleInit, OnModuleDestroy, SagaEventPublisher
{
  ...
  private readonly retryDelayMs = Number(
    process.env.RABBITMQ_RETRY_DELAY_MS ?? 3000,
  );
  private readonly maxRetries = Math.max(
    Number(process.env.RABBITMQ_MAX_RETRIES ?? 10),
    1,
  );
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        if (attempt === this.maxRetries) {
          this.logger.error(
            `RabbitMQ connection failed after ${attempt} attempts: ${message}`,
          );
          throw error;
        }

        this.logger.warn(
          `RabbitMQ is not ready yet (${message}). Retrying in ${this.retryDelayMs}ms...`,
        );
  ...
  async publish(event: SagaEventName, payload: Prisma.JsonValue) {
    await this.publishMessage(event, payload, randomUUID());
  }

  async publishStored(event: OutboxEvent) {
    await this.publishMessage(event.eventType, event.payload, event.id);
  }
  ...
  async consume(
    ...
    await Promise.all(
      keys.map((key) => this.channel!.bindQueue(queue, this.exchange, key)),
    );
    await this.channel.consume(
      ...
      async (message) => {
        ...
        try {
          ...
          this.logger.error(
            `Saga event failed and will be retried: ${error instanceof Error ? error.message : "unknown error"}`,
          );
...
export class RabbitMqSagaConsumer implements OnModuleInit {
  ...
  async onModuleInit() {
    await this.bus.consume(
      "execution-service.saga",
      ["service-order.approved", "service-order.cancelled"],
  ...
  private async handle(message: ConsumeMessage) {
    const event = JSON.parse(message.content.toString()) as {
      event: SagaEventName;
      payload: ApprovedEvent & CancelledEvent;
    };
    if (event.event === "service-order.approved")
      await this.enqueueApproved(event.payload);
    if (event.event === "service-order.cancelled")
      await this.executionService.compensate(
        event.payload.serviceOrderId,
        event.payload.reason || "OS cancelled",
      );
  }
  private async enqueueApproved(event: ApprovedEvent) {
    if (!event.serviceOrderId) throw new Error("serviceOrderId is required");
    try {
      await this.executionService.enqueue(
